...
rule("create_rom", "$objcopy -O verilog --verilog-data-width 16 --only-section=.text* $in $out.tmp && cat $out.tmp | tr ' ' '\\n' | tr -d '\\r' > $out && rm -f $out.tmp", "Creating ROM $out")
rule("create_sram", "$objcopy -O verilog --verilog-data-width 16 --only-section=.data* --only-section=.sdata* --only-section=.bss* --only-section=.sbss* $in $out.tmp && cat $out.tmp | tr ' ' '\\n' | tr -d '\\r' > $out && rm -f $out.tmp", "Creating SRAM $out")
